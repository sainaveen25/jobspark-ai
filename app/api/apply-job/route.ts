...
export async function POST(request: Request) {
  try {
    ...
    const [jobResponse, skillsResponse, experiencesResponse, preferencesResponse, profileResponse, resumeResponse] = await Promise.all([
    ...
    const [jobMatch, application] = await Promise.all([
      ...
      unwrapSupabaseResult(
        await admin
          ...
          .select("*")
          .single(),
        "Unable to apply to job"
      )
    ]);

    return NextResponse.json({ jobMatch, application });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to apply to job" }, { status: 400 });
  }
}
